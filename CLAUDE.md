...
- **gate2(결정론)**: `.claude/hooks/gate2-build-verify.sh`가 `tsc --noEmit` + `npm run lint`(+선택 `next build`)를 돌려 판정. `.claude/settings.json`의 Stop hook에 연결. 기본은 advisory, `GATE_ENFORCE=1`이면 차단.
