...
- **status**: superseded by ADR-0008 (known defect resolved by D-2)
- **date**: retro
...
- **처리(2026-07-09, D-2)**: option (a) 채택. `.dark` 클래스 + 시맨틱 토큰으로 전환했고, `themeStore.ts`는 삭제했다. 현재 테마 결정은 ADR-0008을 따른다.
