...
- (−) 이미지 호스트가 앱의 `remotePatterns`와 어긋날 수 있음 → **D-3에서 실측 종결(2026-07-09)**. PotterDB 전 엔티티 이미지 호스트는 현재 `remotePatterns`와 완전 정합함을 확인했다. 신규 호스트·프록시 불요. Constraints C-4 참조.
