...
i18n 도입 후 Header 액션 영역은 `ThemeToggle` **옆에 언어 스위처**(컴팩트 EN/KO)를 포함한다. 모바일에서는 접힘 메뉴 안에 같은 컨트롤을 미러링한다. 네비 라벨·화면 문구는 활성 로케일 카탈로그로 렌더된다(→ U-10/U-11).
...
## UI 수용기준 (다국어 i18n, 2026-07-10)

> architect 생성, 근거: PRD AC-17~22, ADR-0010(next-intl)·ADR-0011(as-needed 프리픽스), Domain R-6, SDD/i18n. 최종 시각 동치(전환·복원·지속)는 gate3 휴먼 위임(L-8 계승).

- **U-10**: Header 액션 영역(`ThemeToggle` 옆)에 컴팩트 언어 스위처(EN/KO)가 있고, 전환하면 UI 추(chrome)가 대상 로케일 문자열로 다시 렌더된다. 전환은 **현재 경로·필터·페이지를 유지한 채 언어만** 바꾼다(예: `/ko/characters?q=…`). 모바일 접힘 메뉴에 동일 컨트롤이 미러링된다. 선택은 새로고침·재방문 후에도 유지된다(쿠키). → PRD AC-17·20·21. _최종 시각 전환·복원은 gate3 휴먼 위임._
- **U-11**: ko 로케일에서 UI 추 전면(내비·필터/정렬 라벨·검색 플레이스홀더·로딩/에러/빈 상태·버튼·레이아웃·Footer)이 한국어로 렌더되고, 카탈로그를 우회하는 하드코딩 영문이 chrome에 남지 않는다. 고정 도메인 어휘(House 4·Entity 5·Chapter)는 Domain.md R-6 표기와 일치한다. 반면 API 가변 서술문(전기·effect·summary)은 ko에서도 영문 원본을 유지한다. `<html lang>`이 활성 로케일을 반영한다. → PRD AC-18·19·22. _ko chrome 완전성(누락 없음) 시각 확인은 gate3 휴먼 위임._
